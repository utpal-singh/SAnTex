...
where $\sigma_{ij}$ and $\epsilon_{kl}$ are the components of the stress and strain tensors, respectively, while $C_{ijkl}$ is a 4th-order stiffness tensor with 81 elements representing elastic moduli. In this general form, Hooke’s law can account for the anisotropy and directionality of the elastic properties of materials.
...
Within SAnTex, $C_{ijkl}(p, T)$ is the resultant or effective stiffness tensor, $C_{ijkl}(P_0, T_0)$ is the reference stiffness tensor obtained at ambient conditions, with $P_0 = 10^{-4}$~GPa and $T_0 \simeq 298$~K, and $\Delta p$ and $\Delta T$ are deviations from the ambient conditions. The partial derivatives $\frac{\partial C_{ijkl}}{\partial p}$ and $\frac{\partial C_{ijkl}}{\partial T}$ are obtained from the literature sources included in the data package within SAnTex.
...
In the current version of SAnTex, melt is considered as a homogeneously distributed isotropic phase within an anisotropic host rock [@lee_modeling_2017].
...
![3D visualisation of (a) Forsterite Vs splitting, (b) Olivine Vs splitting, (c) Olivine $V_{p}$.](santex_fig_3.png){ width=100% }
